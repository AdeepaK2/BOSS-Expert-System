...
case(1, 'Home-based tuition', expected(proceed, 0.8), [
    ...
    experienced_support(none), reversibility(high),
    ...
    dependency(low), channel_fit(good), legal_status(none_required)
]).
...
case(2, 'Bubble tea outlet', expected(not_recommended_in_this_form, -0.8), [
    ...
    owner_experience(none), owner_time(sufficient),
    experienced_support(none), reversibility(low),
    ...
    dependency(low), channel_fit(good), legal_status(routine)
]).
...
case(3, 'Cloud kitchen', expected(further_validation_required, 0.0), [
    ...
    competition(many_weak), differentiation(strong),
    dependency(high), channel_fit(good), legal_status(unknown)
]).
...
case(5, 'Grocery shop, pawned-jewellery funding', expected(not_recommended, -1.0), [
    ...
    owner_experience(none), owner_time(insufficient),
    experienced_support(none), reversibility(low),
...
case(6, 'Second salon branch', expected(proceed_with_caution, 0.6), [
    ...
    repeat_demand(recurring), price_status(accepted),
    owner_experience(direct), owner_time(sufficient),
    experienced_support(committed), reversibility(high),
    ...
    competition(many_weak), differentiation(strong),
    dependency(high), channel_fit(good), legal_status(routine)
]).
